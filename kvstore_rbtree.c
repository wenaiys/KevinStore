...
void kvs_rbtree_destory(rbtree *tree) {
	...
	if (tree->nil->key) kv_free(tree->nil->key);
...
int kvs_rbtree_set(rbtree *tree, char *key, char *value) {
	...
	node->key = kv_malloc(strlen(key) + 1);
	if (node->key == NULL) {
		kv_free(node);
		return -1;
	}
	...
	node->value = kv_malloc(strlen(value) + 1);
	if (node->value == NULL) {
		kv_free(node->key);
		kv_free(node);
		return -1;
...
int kvs_rbtree_del(rbtree *tree, char *key) {
...
int kvs_rbtree_mod(rbtree *tree, char *key, char *value) {
